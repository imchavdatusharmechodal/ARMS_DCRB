...
function PsReport() {
    ...
    const [dcrbSignature, setDCRBSignature] = useState(null); // State for DCRB signature preview
    ...
    // Handle file selection for DCRB signature
    const handleDCRBSignatureChange = (e) => {
        const file = e.target.files[0];
        if (file) {
            setSelectedFile(file);
            setDCRBSignature(URL.createObjectURL(file)); // Create a preview URL for the selected file
        }
    };

    // Function to upload DCRB signature to the server
    const uploadDCRBSignature = async (file) => {
        const formData = new FormData();
        formData.append('applicationId', id);
        formData.append('DCRBSignature', file);
        ...
        try {
            ...
            if (response.data.status) {
                alert('DCRB signature uploaded successfully!');
                // Refresh the application data to get updated signature path
                await fetchPsReport(id, token);
                return true;
            } else {
                alert('Failed to upload DCRB signature');
                return false;
            }
        } catch (error) {
            console.error('Error uploading DCRB signature:', error);
            alert('Failed to upload DCRB signature');
            return false;
        }
    };
    ...
    const handleSubmit = async () => {
        if (!selectedFile) {
            alert('Please select a DCRB signature file before submitting.');
            return;
        }

        setSubmitting(true);
        try {
            // Upload DCRB signature
            const uploadSuccess = await uploadDCRBSignature(selectedFile);
            
            if (uploadSuccess) {
                // Clear the selected file and preview
                setSelectedFile(null);
                setDCRBSignature(null);
                
                // Reset file input
                const fileInput = document.querySelector('.dcrb-file-input');
                if (fileInput) {
                    fileInput.value = '';
                }
                
                alert('Report submitted successfully with DCRB signature!');
            }
        } catch (error) {
                    ...
                    <div className='table-responsive mt-table'>
                        <table className="table table-bordered custom-table" aria-label="Police Validation Report">
                            <tbody>
                                ...
                                {/* <tr>
                                    <th scope="col">CI हस्ताक्षर</th>
                                    <td scope="col" colSpan={2}>
                                        <div className='signature-img'>
                                            {application?.ci_signature_path ? (
                                                <img
                                                    src={`https://lampserver.uppolice.co.in${application.ci_signature_path}`}
                                                    alt="CI Signature"
                                                    className='signature-img'
                                                    onError={(e) => {
                                                        e.target.style.display = 'none';
                                            ...
                                            <span style={{ display: 'none' }}>CI हस्ताक्षर उपलब्ध नहीं</span>
                                        </div>
                                    </td>
                                </tr> */}
                                <tr>
                                    <th scope="col">DCRB हस्ताक्षर</th>
                                    <td scope="col" colSpan={2}>
                                        <div>
                                            <input
                                                type="file"
                                                className='form-control dcrb-file-input'
                                                accept="image/*"
                                                onChange={handleDCRBSignatureChange}
                                                disabled={submitting}
                                            />
                                            {submitting && <small className="text-muted">Submitting...</small>}
                                        </div>
                                        <div className='signature-img mt-3'>
                                            {dcrbSignature ? (
                                                ...
                                                />
                                            ) : application?.DCRBSignature ? (
                                                <img
                                                    src={`https://lampserver.uppolice.co.in${application.DCRBSignature}`}
                                                    alt="DCRB हस्ताक्षर"
                                                    className='signature-img'
                                                    onError={(e) => {
                                                        e.target.style.display = 'none';
                                                ...
                                                <span>DCRB हस्ताक्षर उपलब्ध नहीं</span>
                                            )}
                                            <span style={{ display: 'none' }}>DCRB हस्ताक्षर उपलब्ध नहीं</span>
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
