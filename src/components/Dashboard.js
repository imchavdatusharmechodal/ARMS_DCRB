...
const Dashboard = () => {
  ...
  const [forwarding, setForwarding] = useState({}); // Track forwarding state for each application

  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 10;

  // New state variables for send modal
    ...
    const fetchApplications = async () => {
      ...
      try {
        ...
        const response = await fetch(
          "https://lampserver.uppolice.co.in/arms/dcrb-applications",
          {
            headers: {
  ...
  // Handle forward to SHO
  const handleForwardToSHO = async (appId) => {
    if (forwarding[appId]) return; // Prevent multiple clicks

    setForwarding(prev => ({ ...prev, [appId]: true }));
    ...
    try {
      ...
      if (response.data.status) {
        ...
        setApplications(prev => 
          prev.map(app => 
            app.id === appId 
              ? { ...app, forwarded_to_sho: true, status: 'forwarded_to_sho' }
              : app
          )
        );
        alert('Successfully forwarded to SHO!');
      } else {
        alert('Failed to forward to SHO');
      }
    } catch (error) {
      console.error('Error forwarding to SHO:', error);
      alert('Failed to forward to SHO. Please try again.');
    } finally {
      setForwarding(prev => ({ ...prev, [appId]: false }));
    }
  };

  // Handle opening send modal
  const handleSendClick = (appId) => {
    setSelectedAppId(appId);
    setSendMessage('');
    setShowSendModal(true);
  };

  // Handle closing send modal
  ...
  // Handle send actions
  const handleSendAction = async (actionType) => {
    if (!selectedAppId || !sendMessage.trim()) {
      alert('Please enter a message before sending.');
      return;
    }

    try {
      // Here you would implement the actual API call based on actionType
      console.log(`Sending ${actionType} for application ${selectedAppId}:`, sendMessage);

      // Example API call structure (uncomment and modify as needed):
      // await axios.post('https://lampserver.uppolice.co.in/arms/send-message', {
      ...
      alert(`Message sent to ${actionType} successfully!`);
                    ...
                    <tbody>
                      {paginatedApplications.map((app, idx) => (
                        <tr key={app.id}>
                          ...
                          <td>
                            {app.forwarded_to_sho ? (
                              <span className="text-success fw-bold">Sent</span>
                            ) : (
                              <button
                                className="btn btn-info btn-sm text-white"
                                onClick={() => handleForwardToSHO(app.id)}
                                disabled={forwarding[app.id]}
                              >
                                <i className="fa-solid fa-paper-plane me-1"></i>
                                {forwarding[app.id] ? 'Sending...' : 'SHO'}
                              </button>
                            )}
                          </td>
